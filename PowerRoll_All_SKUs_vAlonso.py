    ...
    def main(self):
        ...
        for subdir, file_info in dir_files.items():
            if any(file_info.values()):
                ...
                if file_info['summary_file']:
                    print(f"  - Summary: {file_info['summary_file']}")
                    processed_files.append('summary')
                if file_info['socwatch_file']:
                    print(f"  - SoCwatch: {file_info['socwatch_file']}")
                    processed_files.append('socwatch')
                if file_info['results_file']:
                    print(f"  - Results: {file_info['results_file']}")
                    processed_files.append('results')

        print(f"Processed file types: {set(processed_files)}")
        ...
        # Add summary indices if they exist
        if 'summary_indices' in locals() and summary_indices:
            complete_index.extend(summary_indices)

        # Add other indices
        complete_index.extend(soc_index)
        complete_index.extend(browser_efficiency_index)
        complete_index.extend(pch_index)
        complete_index.extend(ioe_pch_index)

        # Ensure the index length matches the DataFrame
        actual_rows = len(Final_data)
        if len(complete_index) == actual_rows:
            Final_data.index = complete_index
        elif len(complete_index) > actual_rows:
            Final_data.index = complete_index[:actual_rows]
        else:
            # Pad with generic names if needed
            while len(complete_index) < actual_rows:
                complete_index.append(f"Unknown_{len(complete_index)}")
            Final_data.index = complete_index

        print(f"DataFrame has {actual_rows} rows, Index has {len(complete_index)} elements")

        # Add debugging before SKU selection
        print(f"Available indices in Final_data:")
        print(Final_data.index.tolist()[:20])  # Show first 20 indices
        print(f"Total indices: {len(Final_data.index)}")
        print()

        if args.sku == "mtl-p":
            available_rails = [rail for rail in mtl_p if rail in Final_data.index]
            missing_rails = [rail for rail in mtl_p if rail not in Final_data.index]
            print(f"MTL-P: Found {len(available_rails)}/{len(mtl_p)} rails")
            if missing_rails:
                print(f"Missing rails: {missing_rails[:5]}...")  # Show first 5 missing
            if available_rails:
                data2 = Final_data.loc[available_rails]
            else:
                data2 = pd.DataFrame()

        elif args.sku == "mtl-m":
            available_rails = [rail for rail in mtl_m if rail in Final_data.index]
            missing_rails = [rail for rail in mtl_m if rail not in Final_data.index]
            print(f"MTL-M: Found {len(available_rails)}/{len(mtl_m)} rails")
            if missing_rails:
                print(f"Missing rails: {missing_rails[:5]}...")
            if available_rails:
                data2 = Final_data.loc[available_rails]
            else:
                data2 = pd.DataFrame()
            ...
            available_rails = [rail for rail in arl_h if rail in Final_data.index]
            missing_rails = [rail for rail in arl_h if rail not in Final_data.index]
            print(f"ARL-H: Found {len(available_rails)}/{len(arl_h)} rails")
            if missing_rails:
                print(f"Missing rails: {missing_rails[:5]}...")
            if available_rails:
                data2 = Final_data.loc[available_rails]
            ...
            available_rails = [rail for rail in arl_u if rail in Final_data.index]
            missing_rails = [rail for rail in arl_u if rail not in Final_data.index]
            print(f"ARL-U: Found {len(available_rails)}/{len(arl_u)} rails")
            if missing_rails:
                print(f"Missing rails: {missing_rails[:5]}...")
            if available_rails:
                data2 = Final_data.loc[available_rails]
            ...
            available_rails = [rail for rail in arl_s if rail in Final_data.index]
            missing_rails = [rail for rail in arl_s if rail not in Final_data.index]
            print(f"ARL-S: Found {len(available_rails)}/{len(arl_s)} rails")
            if missing_rails:
                print(f"Missing rails: {missing_rails[:5]}...")
            if available_rails:
                data2 = Final_data.loc[available_rails]
            ...
            available_rails = [rail for rail in lnl_m if rail in Final_data.index]
            missing_rails = [rail for rail in lnl_m if rail not in Final_data.index]
            print(f"LNL-M: Found {len(available_rails)}/{len(lnl_m)} rails")
            if missing_rails:
                print(f"Missing rails: {missing_rails[:5]}...")
            if available_rails:
                data2 = Final_data.loc[available_rails]
            ...
            available_rails = [rail for rail in ptl_p if rail in Final_data.index]
            missing_rails = [rail for rail in ptl_p if rail not in Final_data.index]
            print(f"PTL-P: Found {len(available_rails)}/{len(ptl_p)} rails")
            if missing_rails:
                print(f"Missing rails: {missing_rails[:5]}...")
            if available_rails:
                data2 = Final_data.loc[available_rails]
            ...
            available_rails = [rail for rail in nvl_s if rail in Final_data.index]
            missing_rails = [rail for rail in nvl_s if rail not in Final_data.index]
            print(f"NVL-S: Found {len(available_rails)}/{len(nvl_s)} rails")
            if missing_rails:
                print(f"Missing rails: {missing_rails[:10]}...")  # Show first 10 missing
            
            power_rails = [idx for idx in Final_data.index if idx.startswith('P_') or idx.startswith('I_') or idx.startswith('V_')]
            print(f"\nActual power rails in your data ({len(power_rails)}):")
            for rail in power_rails[:20]:  # Show first 20
                print(f"  {rail}")
            if len(power_rails) > 20:
                print(f"  ... and {len(power_rails) - 20} more")
            
            if available_rails:
                data2 = Final_data.loc[available_rails]
            else:
                data2 = pd.DataFrame()
                print("No matching power rails found for NVL-S!")
...
if __name__ == '__main__':
    data = CPowerRoll("C:\\Borrar\\WW06_Rerun_Alonso\\")
